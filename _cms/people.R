...
people_current      <- reactive({ get_people_items('current')})
people_community    <- reactive({ get_people_items('community')})
...
output$people_rank_lists_ui <- renderUI({
    bucket_list(
        ...
        add_rank_list(
            text = HTML("<strong>Current Lab Members</strong>"),
            labels = people_current(),
            input_id = "people_rank_list_current"
        ),
        add_rank_list(
            text = HTML("<strong>Community Members</strong>"),
            labels = people_community(),
            input_id = "people_rank_list_community"
        ),
        add_rank_list(
...
observeEvent(input$add_new_person, {
    add_item_alert(
        ...
        callback = function(id, name){
            add_data_yaml_item("people", list(id = id, name = name, status = "current"))
        }
    )
})
    ...
    item_edit_ui("people", "edit_person", function(person){
        tagList(
            fluidRow(
                ...
                column(
                    width = 3,
                    selectInput("edit_person_status", "Status", choices = c("current", "community", "past"), selected = person$status)
                )
            ),
            fluidRow(
                column(
...
observeEvent(input$people_rank_lists, {
    reorder_data_yaml("people", callback = function(cfg){
        people <- list()
        for(status in c('current', 'community', 'past')){
